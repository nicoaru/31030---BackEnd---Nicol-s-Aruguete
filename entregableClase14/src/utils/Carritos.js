...
class Carritos {
    ...
    // METODOS QUE ACTUAN SOBRE PROPIEDAD DEL OBJETO INSTANCIADO
    ...
    addToCart(product, cartId) {
        ...
        return new Promise(async (resolve, reject) => {
            try {
                ...
                if (cartIndex >= 0) {
                    cartsList[cartIndex].productos.push(product)
                    ...
                    resolve({mensaje:`Se agregó el producto con id ${product.id} al carrito ${cartId}`, carrito: cartId, productos: cartsList[cartIndex].productos})
                }
                else if (cartIndex < 0) { 
    ...
    deleteFromCart(productId, cartId) {
        ...
        return new Promise(async (resolve, reject) => {
            try {
                ...
                const productIndex = cartsList[cartIndex].productos.indexOf(cartsList[cartIndex].productos.find(obj => {if (obj.id === productId) {return obj}}));
                
                if (productIndex >= 0) {
                    cartsList[cartIndex].productos.splice(productIndex, 1)
                    await fs.promises.writeFile(`./${fileName}`, JSON.stringify(cartsList, null, 2), "utf-8")
                    console.log(`Se eliminó el producto con id ${productId} del carrito ${cartId}`)
                    resolve({mensaje:`Se eliminó el producto con id ${productId} del carrito ${cartId}`, carrito: cartId, productos: cartsList[cartIndex].productos})
                }
